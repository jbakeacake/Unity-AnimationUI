...
public class AnimationUI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(Application.isPlaying)
#endif
        if(this.PlayOnStart)this.StartCoroutine(this.PlayAnimation());
    }
    ...
    {
        foreach(Sequence sequence in this.AnimationSequence)sequence.Init();
    }
    ...
    public void Play()
    {
        if(!this._isSequencesInitialized)
        {
            this.InitializeSequences();
            this._isSequencesInitialized = true;
        }
        this.Stop();
        this._coroutines.Add(this.StartCoroutine(this.PlayAnimation()));
    }
    ...
    {
        for(int i = 0; i < this.atTimeEvents.Count; i++)this.StartCoroutine(this.AtTimeEvent(this.atTimeEvents[i], this.atTimes[i])); //Function to call at time

        foreach(Sequence sequence in this.AnimationSequence)
        {
            if(sequence.SequenceType == Sequence.Type.Animation)
            {
                ...
                if(sequence.TargetType == Sequence.ObjectType.RectTransform)
                {
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchoredPosition))
                        this._coroutines.Add(this.StartCoroutine(this.TaskAnchoredPosition(sequence.TargetComp.GetComponent<RectTransform>(),
                    ...
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.LocalEulerAngles))
                        this._coroutines.Add(this.StartCoroutine(this.TaskLocalEulerAngles(sequence.TargetComp.GetComponent<RectTransform>(),
                    ...
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.LocalScale))
                        this._coroutines.Add(this.StartCoroutine(this.TaskLocalScale(sequence.TargetComp.GetComponent<RectTransform>(),
                    ...
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchorMax))
                        this._coroutines.Add(this.StartCoroutine(this.TaskAnchorMax(sequence.TargetComp.GetComponent<RectTransform>(),
                    ...
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchorMin))
                        this._coroutines.Add(this.StartCoroutine(this.TaskAnchorMin(sequence.TargetComp.GetComponent<RectTransform>(),
                    ...
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.SizeDelta))
                        this._coroutines.Add(this.StartCoroutine(this.TaskSizeDelta(sequence.TargetComp.GetComponent<RectTransform>(),
                    ...
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.Pivot))
                        this._coroutines.Add(this.StartCoroutine(this.TaskPivot(sequence.TargetComp.GetComponent<RectTransform>(),
                ...
                else if(sequence.TargetType == Sequence.ObjectType.Transform)
                {
                    if(sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalPosition))
                        this._coroutines.Add(this.StartCoroutine(this.TaskLocalPosition(sequence.TargetComp.transform,
                    ...
                    if(sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalEulerAngles))
                        this._coroutines.Add(this.StartCoroutine(this.TaskLocalEulerAngles(sequence.TargetComp.transform,
                            sequence.LocalEulerAnglesStart, sequence.LocalEulerAnglesEnd, sequence.Duration, sequence.EaseFunction
                        )));
                    if(sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalScale))
                        this._coroutines.Add(this.StartCoroutine(this.TaskLocalScale(sequence.TargetComp.transform,
                            sequence.LocalScaleStart, sequence.LocalScaleEnd, sequence.Duration, sequence.EaseFunction
                        )));
                }
                else if(sequence.TargetType == Sequence.ObjectType.Image)
                {
                    if(sequence.TargetImgTask.HasFlag(Sequence.ImgTask.Color))
                        this._coroutines.Add(this.StartCoroutine(this.TaskColor(sequence.TargetComp.GetComponent<Image>(),
                    ...
                    if(sequence.TargetImgTask.HasFlag(Sequence.ImgTask.FillAmount))
                        this._coroutines.Add(this.StartCoroutine(this.TaskFillAmount(sequence.TargetComp.GetComponent<Image>(),
                ...
                else if(sequence.TargetType == Sequence.ObjectType.CanvasGroup)
                {
                    if(sequence.TargetCgTask.HasFlag(Sequence.CgTask.Alpha))
                        this._coroutines.Add(this.StartCoroutine(this.TaskAlpha(sequence.TargetComp.GetComponent<CanvasGroup>(),
                ...
                else if(sequence.TargetType == Sequence.ObjectType.Camera)
                {
                    if(sequence.TargetCamTask.HasFlag(Sequence.CamTask.BackgroundColor))
                        this._coroutines.Add(this.StartCoroutine(this.TaskBackgroundColor(sequence.TargetComp.GetComponent<Camera>(),
                    ...
                    if(sequence.TargetCamTask.HasFlag(Sequence.CamTask.OrthographicSize))
                        this._coroutines.Add(this.StartCoroutine(this.TaskOrthographicSize(sequence.TargetComp.GetComponent<Camera>(),
                ...
                else if(sequence.TargetType == Sequence.ObjectType.TextMeshPro)
                {
                    if(sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.Color))
                        this._coroutines.Add(this.StartCoroutine(this.TaskTextMeshProColor(sequence.TargetComp.GetComponent<TMP_Text>(),
                    ...
                    if(sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.MaxVisibleCharacters))
                        this._coroutines.Add(this.StartCoroutine(this.TaskMaxVisibleCharacters(sequence.TargetComp.GetComponent<TMP_Text>(),
            ...
            {
                this.SetActiveAllInput(sequence.IsActivating);
            }
            ...
            else if(sequence.SequenceType == Sequence.Type.SFX)
            {
                if(sequence.PlaySFXBy == Sequence.SFXMethod.File)
                {
                    ...
                    this.PlaySound(sequence.SFXFile);
                }
                else // if(sequence.PlaySFXBy == Sequence.SFXMethod.Index)
                    this.PlaySound(sequence.SFXIndex);
            }
    ...
    IEnumerator ReverseArrayAtTime(float t)
    {
        ...
        Array.Reverse(this.AnimationSequence);
    }
    IEnumerator PlayReversedAnimation()
    {
        Array.Reverse(this.AnimationSequence);
        this.ReverseArrayAtTime(this.TotalDuration);
...
#region Event
    ...
    {
        this.atTimes.Add(time);
        this.atTimeEvents.Add(func);
        return this;
    }
...
#region Overidable
...
#if UNITY_EDITOR
    void OnEnable() => UnityEditor.EditorApplication.update += this.EditorUpdate;
    void OnDisable() => UnityEditor.EditorApplication.update -= this.EditorUpdate;
    ...
    void EditorUpdate()
    {
        ...
        if(this.IsPlayingInEditMode && this.CurrentTime < this.TotalDuration)
        {
            this.CurrentTime = Mathf.Clamp(Time.time - this._startTime, 0, this.TotalDuration);
            this.UpdateSequence(this.CurrentTime);
        }
        else
        {
            if(this.UpdateSequence != null && this.IsPlayingInEditMode)this.UpdateSequence(this.TotalDuration); //Make sure the latest frame is called
            this.IsPlayingInEditMode = false;
        }
    }
    public void UpdateBySlider()
    {
        if(Application.isPlaying)return;
        if(this.IsPlayingInEditMode)return;
        this.InitFunction();
        if(this.UpdateSequence != null)this.UpdateSequence(this.CurrentTime);
    }
    ...
    public void PreviewAnimation()
    {
        ...
        this.CurrentTime = 0;
        this.IsPlayingInEditMode = true;
        this.UpdateSequence(0);// Make sure the first frame is called
    }
    ...
    public void PreviewEnd()
    {
        ...
        this.IsPlayingInEditMode = false;
        this.CurrentTime = this.TotalDuration;
        this.UpdateSequence(Mathf.Clamp01(this.TotalDuration));
    ...
    {
        this.AnimationSequence = new Sequence[1]
        {
            new Sequence()
        };
    }
...
#region timing
    public void InitTime()
    {
        ...
        foreach(Sequence sequence in this.AnimationSequence)
        {
            this.TotalDuration += (sequence.SequenceType == Sequence.Type.Wait) ? sequence.Duration : 0;
        }
        // for case when the duration of a non wait is bigger
        float currentTimeCheck = 0;
        float tempTotalDuration = this.TotalDuration;
        foreach(Sequence sequence in this.AnimationSequence)
        {
            currentTimeCheck += (sequence.SequenceType == Sequence.Type.Wait) ? sequence.Duration : 0;
            if(sequence.SequenceType == Sequence.Type.Animation)
            {
                if(this.TotalDuration < currentTimeCheck + sequence.Duration)
                {
                    this.TotalDuration = currentTimeCheck + sequence.Duration;
                }
            }
        }
        this.CurrentTime = Mathf.Clamp(this.CurrentTime, 0, this.TotalDuration);
    }
#endregion timing
        ...
        foreach(Sequence sequence in this.AnimationSequence)
        {
            ...
            if(sequence.SequenceType == Sequence.Type.Animation)
            {
                ...
                if(sequence.TargetType == Sequence.ObjectType.RectTransform)
                {
                    ...
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchoredPosition))
                        this.UpdateSequence += RtAnchoredPosition;
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.LocalEulerAngles))
                        this.UpdateSequence += RtLocalEulerAngles;
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.LocalScale))
                        this.UpdateSequence += RtLocalScale;
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.SizeDelta))
                        this.UpdateSequence += RtSizeDelta;
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchorMax))
                        this.UpdateSequence += RtAnchorMax;
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.AnchorMin))
                        this.UpdateSequence += RtAnchorMin;
                    if(sequence.TargetRtTask.HasFlag(Sequence.RtTask.Pivot))
                        this.UpdateSequence += RtPivot;
                ...
                else if(sequence.TargetType == Sequence.ObjectType.Transform)
                {
                    ...
                    if(sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalPosition))
                        this.UpdateSequence += TransLocalPosition;
                    if(sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalEulerAngles))
                        this.UpdateSequence += TransLocalEulerAngles;
                    if(sequence.TargetTransTask.HasFlag(Sequence.TransTask.LocalScale))
                        this.UpdateSequence += TransLocalScale;
                }
                else if(sequence.TargetType == Sequence.ObjectType.Image)
                {
                    ...
                    if(sequence.TargetImgTask.HasFlag(Sequence.ImgTask.Color))
                        this.UpdateSequence += ImgColor;
                    if(sequence.TargetImgTask.HasFlag(Sequence.ImgTask.FillAmount))
                        this.UpdateSequence += ImgFillAmount;
                }
                else if(sequence.TargetType == Sequence.ObjectType.CanvasGroup)
                {
                ...
                else if(sequence.TargetType == Sequence.ObjectType.Camera)
                {
                    ...
                    if(sequence.TargetCamTask.HasFlag(Sequence.CamTask.BackgroundColor))
                        this.UpdateSequence += CamBackgroundColor;
                    if(sequence.TargetCamTask.HasFlag(Sequence.CamTask.OrthographicSize))
                        this.UpdateSequence += CamOrthographicSize;
                }
                else if(sequence.TargetType == Sequence.ObjectType.TextMeshPro)
                {
                    ...
                    if(sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.Color))
                        this.UpdateSequence += TextMeshProColor;
                    if(sequence.TargetTextMeshProTask.HasFlag(Sequence.TextMeshProTask.MaxVisibleCharacters))
                        this.UpdateSequence += MaxVisibleCharacters;
                }
                else if(sequence.TargetType == Sequence.ObjectType.UnityEventDynamic)
                {
                    ...
                    this.UpdateSequence += EventDynamic;
                }
            ...
            else if(sequence.SequenceType == Sequence.Type.UnityEvent)
            {
                ...
                sequence.IsDone = false;
                this.UpdateSequence += UnityEvent;
            }
        }
    }
#endif
