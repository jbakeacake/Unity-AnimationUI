...
public class SequenceDrawer : PropertyDrawer
{
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        ...
        {
            property.FindPropertyRelative("PropertyRectHeight").floatValue = this._height;
            return this._height;
        }
        ...
        if(sequenceType == Sequence.Type.Animation)
        {
            ...
            {
                totalHeight = this._height*5 + EditorGUI.GetPropertyHeight(property.FindPropertyRelative("EventDynamic")) + EditorGUIUtility.standardVerticalSpacing;

                property.FindPropertyRelative("PropertyRectHeight").floatValue = totalHeight;
                return totalHeight;
            }

            if(property.FindPropertyRelative("TargetComp").GetSerializedValue<Component>() == null)return this._height*6;
            ...
            if(objectType == Sequence.ObjectType.RectTransform)
            {
                ...
                if(rtTask.HasFlag(Sequence.RtTask.AnchoredPosition))totalHeight+=this._height*3;
                if(rtTask.HasFlag(Sequence.RtTask.LocalScale))totalHeight+=this._height*3;
                if(rtTask.HasFlag(Sequence.RtTask.LocalEulerAngles))totalHeight+=this._height*3;
                if(rtTask.HasFlag(Sequence.RtTask.SizeDelta))totalHeight+=this._height*3;
                if(rtTask.HasFlag(Sequence.RtTask.AnchorMax))totalHeight+=this._height*3;
                if(rtTask.HasFlag(Sequence.RtTask.AnchorMin))totalHeight+=this._height*3;
                if(rtTask.HasFlag(Sequence.RtTask.Pivot))totalHeight+=this._height*3;
            }
            else if(objectType == Sequence.ObjectType.Transform)
            {
                ...
                if(transTask.HasFlag(Sequence.TransTask.LocalPosition))totalHeight+=this._height*3;
                if(transTask.HasFlag(Sequence.TransTask.LocalScale))totalHeight+=this._height*3;
                if(transTask.HasFlag(Sequence.TransTask.LocalEulerAngles))totalHeight+=this._height*3;
            }
            else if(objectType == Sequence.ObjectType.Image)
            {
                ...
                if(imgTask.HasFlag(Sequence.ImgTask.Color))totalHeight+=this._height*3;
                if(imgTask.HasFlag(Sequence.ImgTask.FillAmount))totalHeight+=this._height*3;
            }
            else if(objectType == Sequence.ObjectType.CanvasGroup)
            {
                ...
                if(cgTask.HasFlag(Sequence.CgTask.Alpha))totalHeight+=this._height*3;
            }
            else if(objectType == Sequence.ObjectType.Camera)
            {
                ...
                if(camTask.HasFlag(Sequence.CamTask.BackgroundColor))totalHeight+=this._height*3;
                if(camTask.HasFlag(Sequence.CamTask.OrthographicSize))totalHeight+=this._height*3;
            }
            else if(objectType == Sequence.ObjectType.TextMeshPro)
            {
                ...
                if(textMeshProTask.HasFlag(Sequence.TextMeshProTask.Color))totalHeight+=this._height*3;
                if(textMeshProTask.HasFlag(Sequence.TextMeshProTask.MaxVisibleCharacters))totalHeight+=this._height*3;
            }




            property.FindPropertyRelative("PropertyRectHeight").floatValue = this._height*6 + totalHeight;
            return this._height*6 + totalHeight +1;
        }
#region others
        else if(sequenceType == Sequence.Type.Wait)
        {
            property.FindPropertyRelative("PropertyRectHeight").floatValue = this._height * 3;
        ...
        {
            property.FindPropertyRelative("PropertyRectHeight").floatValue = this._height * 4;
        ...
        {
            property.FindPropertyRelative("PropertyRectHeight").floatValue = this._height + (EditorGUI.GetPropertyHeight(property.FindPropertyRelative("Event"))+EditorGUIUtility.singleLineHeight);
            return this._height + (EditorGUI.GetPropertyHeight(property.FindPropertyRelative("Event"))+EditorGUIUtility.singleLineHeight);
        }

        property.FindPropertyRelative("PropertyRectHeight").floatValue = this._height * 7;
        return this._height * 7; //Not gonna happen
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        Rect nextPosition = new Rect(position.x, position.y, position.width, this._height);

        Rect backgroundRect = new Rect(this._backgroundRectExtra.x, position.y+this._backgroundRectExtra.y, position.width+this._backgroundRectExtra.width, this.GetPropertyHeight(property, label)+this._backgroundRectExtra.height);
...
#region label
        ...
        if(GUI.Button(new Rect(position.x+position.width-this._buttonWidth*2, position.y-3, this._buttonWidth, this._height), "Start"))
        {
            property.FindPropertyRelative("TriggerStart").boolValue = true;
        }
        else if(GUI.Button(new Rect(position.x+position.width-this._buttonWidth, position.y-3, this._buttonWidth, this._height), "End"))
        {
...
#region setup animation
            ...
            if(objectType == Sequence.ObjectType.RectTransform)
            {
                ...
                {
                    nextPosition.y += this._height;
                    EditorGUI.LabelField(new Rect(nextPosition.x, nextPosition.y, nextPosition.width, this._height),
                        new GUIContent(name)
                    );

                    nextPosition.y += this._height;
                    if(GUI.Button(new Rect(nextPosition.x, nextPosition.y, nextPosition.width/4-5, this._height),"Set Start"))
                    {
                    ...
                    EditorGUI.PropertyField(
                        new Rect(nextPosition.x+nextPosition.width/4, nextPosition.y, nextPosition.width*3/4, this._height),
                        property.FindPropertyRelative(name+"Start"), GUIContent.none
                    );

                    nextPosition.y += this._height;
                    if(GUI.Button(new Rect(nextPosition.x, nextPosition.y, nextPosition.width/4-5, this._height),"Set End"))
                    {
